...
static inline bool ieee80211_sn_less(__u16 sn1, __u16 sn2)
{
	return ((sn1 - sn2) & IEEE80211_SN_MASK) > (IEEE80211_SN_MODULO >> 1);
}

static inline __u16 ieee80211_sn_add(__u16 sn1, __u16 sn2)
{
	return (sn1 + sn2) & IEEE80211_SN_MASK;
}

static inline __u16 ieee80211_sn_inc(__u16 sn)
{
	return ieee80211_sn_add(sn, 1);
}

static inline __u16 ieee80211_sn_sub(__u16 sn1, __u16 sn2)
{
	return (sn1 - sn2) & IEEE80211_SN_MASK;
}
...
struct ieee80211s_hdr {
	...
	__u8 ttl;
	__le32 seqnum;
	__u8 eaddr1[ETH_ALEN];
	__u8 eaddr2[ETH_ALEN];
};
...
struct ieee80211_wide_bw_chansw_ie {
	__u8 new_channel_width;
	__u8 new_center_freq_seg0, new_center_freq_seg1;
} __packed;
...
struct ieee80211_meshconf_ie {
	__u8 meshconf_psel;
	__u8 meshconf_pmetric;
	__u8 meshconf_congest;
	__u8 meshconf_synch;
	__u8 meshconf_auth;
	__u8 meshconf_form;
	__u8 meshconf_cap;
	...
	union {
		...
		struct {
			...
			union {
				...
				struct {
					__u8 action_code;
					__u8 membership[WLAN_MEMBERSHIP_LEN];
					__u8 position[WLAN_USER_POSITION_LEN];
				} __packed vht_group_notif;
				struct {
					__u8 action_code;
					__u8 dialog_token;
					__u8 tpc_elem_id;
					__u8 tpc_elem_length;
...
struct ieee80211_wmm_ac_param {
	__u8 aci_aifsn; /* AIFSN, ACM, ACI */
	__u8 cw; /* ECWmin, ECWmax (CW = 2^ECW - 1) */
	__le16 txop_limit;
} __packed;

struct ieee80211_wmm_param_ie {
	__u8 element_id; /* Element ID: 221 (0xdd); */
	__u8 len; /* Length: 24 */
	/* required fields for WMM version 1 */
	__u8 oui[3]; /* 00:50:f2 */
	__u8 oui_type; /* 2 */
	__u8 oui_subtype; /* 1 */
	__u8 version; /* 1 for WMM version 1.0 */
	__u8 qos_info; /* AP/STA specific QoS info */
	__u8 reserved; /* 0 */
...
struct ieee80211_tdls_lnkie {
	__u8 ie_type; /* Link Identifier IE */
	__u8 ie_len;
	__u8 bssid[ETH_ALEN];
	__u8 init_sta[ETH_ALEN];
	__u8 resp_sta[ETH_ALEN];
} __packed;
...
struct ieee80211_vht_operation {
	__u8 chan_width;
	__u8 center_freq_seg0_idx;
	__u8 center_freq_seg1_idx;
...
#define IEEE80211_VHT_CAP_SUPP_CHAN_WIDTH_160_80PLU__s80MHZ	0x00000008
...
#define IEEE80211_HE_PHY_CAP0_CHANNEL_WIDTH_SET_80PLU__s80_MHZ_IN_5G	0x10
...
ieee80211_he_mcs_nss_size(const struct ieee80211_he_cap_elem *he_cap)
{
	...
	if (he_cap->phy_cap_info[0] &
	    IEEE80211_HE_PHY_CAP0_CHANNEL_WIDTH_SET_80PLU__s80_MHZ_IN_5G)
		count += 4;
 ...
 * Input: Header byte of ppe_thres (first byte), and HE capa IE's PHY cap __u8*
 */
static inline __u8
ieee80211_he_ppe_size(__u8 ppe_thres_hdr, const __u8 *phy_cap_info)
{
	__u8 n;
 ...
 */
static inline __u8
ieee80211_he_oper_size(const __u8 *he_oper_ie)
{
	struct ieee80211_he_operation *he_oper = (void *)he_oper_ie;
	__u8 oper_len = sizeof(struct ieee80211_he_operation);
	u32 he_oper_params;
 ...
 */
static inline __u8
ieee80211_he_spr_size(const __u8 *he_spr_ie)
{
	struct ieee80211_he_spr *he_spr = (void *)he_spr_ie;
	__u8 spr_len = sizeof(struct ieee80211_he_spr);
	u32 he_spr_params;
 ...
 * 4 addr: 2 + 2 + 2 + 4*6 = 30
 */
static inline __u8 *ieee80211_get_qos_ctl(struct ieee80211_hdr *hdr)
{
 ...
 * @hdr: the frame
 */
static inline __u8 ieee80211_get_tid(struct ieee80211_hdr *hdr)
{
	__u8 *qc = ieee80211_get_qos_ctl(hdr);
 ...
 * field.
 */
static inline __u8 *ieee80211_get_SA(struct ieee80211_hdr *hdr)
{
	if (ieee80211_has_a4(hdr->frame_control))
		return hdr->addr4;
 ...
 * field.
 */
static inline __u8 *ieee80211_get_DA(struct ieee80211_hdr *hdr)
{
...
static inline bool ieee80211_check_tim(const struct ieee80211_tim_ie *tim,
				       __u8 tim_len, __u16 aid)
{
	__u8 mask;
	__u8 index, indexn1, indexn2;
...
static inline int ieee80211_get_tdls_action(struct sk_buff *skb, u32 hdr_size)
{
	if (!skb_is_nonlinear(skb) &&
		...
		const __u8 *tdls_data = skb_network_header(skb) - 2;
...
#define for_each_element(_elem, _data, _datalen)			\
	for (_elem = (const struct element *)(_data);			\
	     (const __u8 *)(_data) + (_datalen) - (const __u8 *)_elem >=	\
		(int)sizeof(*_elem) &&					\
	     (const __u8 *)(_data) + (_datalen) - (const __u8 *)_elem >=	\
...
{
	return (const __u8 *)element == (const __u8 *)data + datalen;
}
